...
public class NetworkProximityGridChecker : NetworkBehaviour
{
    ...
    // Unity is single threaded. use one HashSet for all connections, this way
    // we save large amounts of allocations/GC.
    HashSet<NetworkConnection> rebuildCache = new HashSet<NetworkConnection>();
    public override bool OnRebuildObservers(HashSet<NetworkConnection> observers, bool initial)
    {
        ...
        Vector2Int current = ProjectToGrid(transform.position);
        grid.GetWithNeighbours(current, rebuildCache);
        observers.UnionWith(rebuildCache);
